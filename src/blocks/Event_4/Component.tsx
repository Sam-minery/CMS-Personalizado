        ...
        <div className="mx-auto flex w-full max-w-lg flex-col">
          ...
          <Tabs defaultValue={tabs[0].value} className="flex flex-col justify-start">
            ...
            {tabs.map((tab, tabIndex) => (
              <TabsContent
                key={`tab-content-${tab.value}-${tabIndex}`}
                value={tab.value}
                className="data-[state=active]:animate-tabs"
              >
                <div className="flex flex-col gap-6 md:gap-8">
                  {tab.content.map((event, index) => (
                    <FeaturedEvent key={`event-${tab.value}-${index}`} {...event} />
                  ))}
                </div>
              </TabsContent>
            ))}
...
const FeaturedEvent: React.FC<FeaturedEvent> = ({
  ...
  const imageUrl = typeof image === 'string' ? image : image?.url || 'https://d22po4pjz3o32e.cloudfront.net/placeholder-image.svg'
  ...
  return (
    <div className="flex flex-col border border-border-primary md:flex-row">
      ...
      <div className="flex w-full flex-col items-start gap-8 p-6 sm:p-8 lg:flex-row lg:items-center">
        ...
        <Button {...button} className="flex items-center justify-center gap-x-2" asChild>
          <a href={button.url}>
            {button.title}
          </a>
        </Button>
