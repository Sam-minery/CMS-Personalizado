...
export const Comparison13 = (props: Comparison13Props) => {
  ...
  // Convert products to handle media uploads and feature items
  const convertedProducts = products.map(product => ({
    ...
    mainFeatures: product.mainFeatures.map(feature => ({
      ...feature,
      items: convertFeatureItems(Array.isArray(feature.items) ? feature.items : [feature.items])
    })),
        ...
        {product.mainFeatures.map((feature, index) => (
          ...
          >
            ...
            <h6 className="text-md font-bold leading-[1.4] md:text-lg md:leading-[1.4]">
              {Array.isArray(feature.items) ? feature.items[0] : feature.items}
            </h6>
          </div>
        ))}
      </div>
