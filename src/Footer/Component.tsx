...
export async function Footer() {
  ...
  // Si el tipo de footer es footerTemplate, renderizar el componente FooterTemplate
  if (footerData?.footerType === 'footerTemplate' && footerData?.footerTemplateConfig) {
    return (
      <FooterTemplate
        logo={{
          image: footerData.footerTemplateConfig.logo?.image,
          text: footerData.footerTemplateConfig.logo?.text || 'Logo',
          link: mapPayloadLinkToComponentLink(footerData.footerTemplateConfig.logo?.link),
        }}
        subheading={footerData.footerTemplateConfig.subheading || 'Safe, observable, outcome-driven AI'}
        ctaButton={{
          title: footerData.footerTemplateConfig.ctaButton?.title || 'Start a 30-day trial',
          link: mapPayloadLinkToComponentLink(footerData.footerTemplateConfig.ctaButton?.link),
        }}
        productLinks={footerData.footerTemplateConfig.productLinks?.map(item => ({
          title: item.title,
          link: mapPayloadLinkToComponentLink(item.link),
        })) || []}
        companyLinks={footerData.footerTemplateConfig.companyLinks?.map(item => ({
          title: item.title,
          link: mapPayloadLinkToComponentLink(item.link),
        })) || []}
        newsletterDescription={footerData.footerTemplateConfig.newsletterDescription || 'Get the latest product news and behind the scenes updates.'}
        newsletterPlaceholder={footerData.footerTemplateConfig.newsletterPlaceholder || 'Your email'}
        footerText={footerData.footerTemplateConfig.footerText || `© ${new Date().getFullYear()} Agenforce AI. All rights reserved.`}
        footerLinks={footerData.footerTemplateConfig.footerLinks?.map(item => ({
          title: item.title,
          link: mapPayloadLinkToComponentLink(item.link),
        })) || []}
        socialLinks={footerData.footerTemplateConfig.socialLinks?.map(social => ({
          platform: social.platform as 'twitter' | 'instagram' | 'linkedin' | null,
          link: mapPayloadLinkToComponentLink(social.link),
        })) || []}
      />
    )
  }
