...
export const Footer4: React.FC<Footer4Props> = (props) => {
    ...
    <footer id="relume" className="px-[5%] py-12 md:py-18 lg:py-20">
      <div className="container">
        <div className="grid grid-cols-1 items-center justify-center justify-items-center gap-x-[4vw] gap-y-12 pb-12 md:pb-18 lg:grid-cols-[0.25fr_1fr_0.25fr] lg:justify-between lg:gap-y-4 lg:pb-20">
          <CMSLink url={logo.url || '#'} className="lg:justify-self-start">
            {logo.src && (
              <Image 
                src={logo.src} 
                alt={logo.alt || 'Logo'} 
            ...
            >
              {column.links.map((link, linkIndex) => (
                <li key={linkIndex} className="font-semibold">
                  <CMSLink url={link.url} appearance="link">
                    {link.title}
                  </CMSLink>
                </li>
              ))}
          ...
          <div className="flex items-start justify-start justify-items-center gap-x-3 lg:justify-self-end">
            {socialMediaLinks.map((link, index) => (
              <CMSLink key={index} url={link.url} appearance="link">
                {link.icon}
              </CMSLink>
            ))}
          </div>
