...
const sendaBlockButtonBoxCore =
  'inline-flex items-center justify-center gap-2 rounded-2xl whitespace-nowrap w-auto max-w-full shrink-0'

/** Con ui/Button o CMSLink con apariencia de botón: anula h-9/px-3 y `rounded` del variant. `!rounded-2xl` refuerza frente a estilos base del ui/button. */
export const sendaBlockButtonPrimitiveClassName = cn(
  sendaBlockButtonBoxCore,
  '!rounded-2xl !h-auto !min-h-[40px] md:!min-h-[48px] !pl-4 !pr-2.5 md:!pl-5 md:!pr-3 !py-0',
)

/** Botón nativo <button> o CMSLink appearance="inline" (solo enlace). */
export const sendaBlockButtonNativeClassName = cn(
  sendaBlockButtonBoxCore,
  '!rounded-2xl min-h-[40px] md:min-h-[48px] pl-4 pr-2.5 md:pl-5 md:pr-3 py-0',
)
...
export const sendaBlockButtonNativeSymmetricClassName = cn(
  sendaBlockButtonBoxCore,
  '!rounded-2xl min-h-[40px] md:min-h-[48px] px-4 md:px-5 py-0',
)
