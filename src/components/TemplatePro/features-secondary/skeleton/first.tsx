...
export const SkeletonOne = () => {
  ...
  return (
    <motion.div
      ref={ref}
      layout
      className="flex-1 rounded-t-3xl gap-2 flex flex-col bg-neutral-100 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 max-w-[20rem] lg:max-w-sm mx-auto w-full h-full absolute inset-0 p-2"
    >
...
const Card = ({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "size-6 shrink-0 rounded-full flex mt-1 items-center justify-center relative"
        )}
        style={{
          backgroundColor:
...
const Tag = ({ text, icon }: { text: string; icon: React.ReactNode }) => {
  return (
    <div className="flex items-center gap-1 w-fit rounded-sm px-1 py-0.5 border border-neutral-200 dark:border-neutral-700 text-sm">
      <span className="text-neutral-600 dark:text-neutral-400 [&>svg]:fill-current [&>svg]:stroke-current">{icon}</span>
